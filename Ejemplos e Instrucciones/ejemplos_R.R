...
rem_ipc_2026_serie <- rem_ipc_2026$observaciones %>% 
  mutate(fecha=as.Date(fecha)) %>% 
  filter(fecha=="2026-01-01")

# Expectativa de inflación anual para 2026 (REM) - último dato
rem_ipc_2026_ult <- rem_ipc_2026$observaciones %>% 
  mutate(fecha=as.Date(fecha)) %>%
  filter(fecha=="2026-01-01",realtime_end=="9999-12-31")

# Expectativa de inflación anual para 2026 (REM) vigente a principios de año
rem_ipc_2026_ene26 <- rem_ipc_2026$observaciones %>% 
  mutate(fecha=as.Date(fecha)) %>%
  filter(fecha=="2026-01-01",realtime_end>="2026-01-01") %>% 
