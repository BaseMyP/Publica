# ==============================================================================
# SCRIPT DE ACTUALIZACIÓN: Tasas De préstamos al sector privado no financiero
# ==============================================================================
...
message("Iniciando actualización semanal de preser_tas.xls: ", Sys.time())

# 1. Validar Catálogo
cat_path <- "catalogo.json"
catalogo_completo <- fromJSON(cat_path)

catalogo_preser_tas <- catalogo_completo %>% filter(metodo_etl == "EXCEL_PRESER_TAS")

if (nrow(catalogo_preser_tas) == 0) {
  message("No hay series configuradas para EXCEL_PRESER_TAS. Finalizando.")
  quit(save = "no")
}
...
for (i in 1:nrow(catalogo_preser_tas)) {
  serie_id <- catalogo_preser_tas$serie_id[i]
  tema <- basename(dirname(catalogo_preser_tas$raw_url[i]))
  ...
  if (file.exists(path_archivo)) {
    ...
    nuevo_df <- df_raw %>%
      ...
      select(fecha = 3, valor = all_of(col_index)) %>%
